...
Die Bézierkurve ist definiert durch
\[
\mathbf{B}(t) = \sum_{i=0}^{n} \binom{n}{i} (1 - t)^{n-i} t^{i} \mathbf{P}_i,
\quad t \in [0,1]
\]
sie ist eine glatte, parametrische Kurve, die durch Kontrollpunkte beschrieben wird. Sie beginnt im ersten und endet im letzten Punkt, während die übrigen Punkte den Verlauf beeinflussen. Mit wenigen Kontrollpunkten ist sie leicht zu berechnen, stabil und gut kontrollierbar. Bei vielen Kontrollpunkten steigt jedoch der Polynomgrad, was den Rechenaufwand erhöht, zu numerischen Problemen führen kann und die lokale Kontrolle verschlechtert, da Änderungen an einem Punkt die gesamte Kurve beeinflussen. Deshalb verwendet man in der Praxis bei komplexen Formen meist mehrere Bézierkurven niedrigen Grades oder alternative Kurven wie Splines.
\\
TODO!!! abbildung hinzufügen
\subsubsection*{Fazit}
...
TODO!!! Lenkwinkel berechnen ohne dass Reifen mit Fahrzeuggehäuse kollidieren
\end{document}