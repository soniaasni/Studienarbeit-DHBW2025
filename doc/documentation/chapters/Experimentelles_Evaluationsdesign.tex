\section{Experimentelles Evaluationsdesign} Zur Bewertung der entwickelten Lösung wurden standardisierte Testszenarien definiert. Ziel war die Untersuchung der Zuverlässigkeit der Hinderniserkennung sowie der autonomen Spurplanung. Hierfür wurden drei Testszenarien festgelegt:
\begin{itemize}
  \item Szenario A: Einzelnes Hindernis mittig auf der Fahrbahn
  \item Szenario B: Hindernis rechts versetzt zur Fahrbahn
  \item Szenario C: Zwei aufeinanderfolgende Hindernisse
\end{itemize}
Jedes Szenario wurde 10-mal durchgeführt. Erfasst wurden die Erfolgsquote der Hinderniserkennung sowie die Erfolgsquote des Ausweichmanövers. Die Ergebnisse werden in Kapitel 10 quantitativ ausgewertet und mit den zuvor definierten Projektzielen verglichen.

\subsection{Methoden zur Hindernisumfahrung}
...
Durch diese rekursive Überlagerung entsteht eine adaptive Fahrtenkurve, die sich flexibel an nahezu jede Umgebung anpassen kann. Unter geeigneten Randbedingungen kann die rekursive Anwendung mehrerer Gaußfunktionen die Umfahrung komplexer Hindernisstrukturen unterstützen.
\newpage

\subsubsection*{Fazit}
Zusammenfassend lässt sich sagen, dass die Gaußsche Glockenkurve aufgrund ihrer symmetrischen Form, der lokalen Begrenzung, einfachen Parametrierbarkeit und der sanften Krümmung besonders gut geeignet ist für die Umfahrung von Gegenständen. Parabeln können in bestimmten Situationen nützlich sein, bieten jedoch nicht die notwendige Flexibilität und Kontrolle. Exponentialfunktionen sind für diesen Anwendungsfall weniger geeignet, da sie zu einseitig und abrupt reagieren. Die Möglichkeit der rekursiven Anwendung der Gaußschen Glockenkurve ermöglicht es zudem, komplexe Hindernisformationen effektiv zu bewältigen, was sie zur optimalen Lösung für die Fahrbahnplanung bei der Umfahrung von Gegenständen macht.
...
Die Berechnung des Lenkwinkels für die Umfahrung eines Gegenstandes basiert auf der geometrischen Beziehung zwischen der Fahrbahnkurve und der Fahrzeugdynamik. Angenommen, die Fahrbahn wird durch eine Funktion \(f(x)\) beschrieben, die die seitliche Auslenkung in Abhängigkeit von der Vorwärtsbewegung \(x\) angibt, kann der Lenkwinkel \(\delta\) wie folgt approximiert werden:
