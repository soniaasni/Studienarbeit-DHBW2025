...
\subsection{Bestehende funktionale Komponenten}
Zu Beginn der Studienarbeit existierte bereits ein teilweise funktionsfähiger Prototyp des Modellfahrzeugs. Die grundlegende Fahrzeugplattform war aufgebaut und mit einem Raspberry Pi 4 als zentralem Steuerrechner ausgestattet. Weiterhin war bereits ein \gls{lidar}-Sensor mechanisch am Fahrzeug montiert und elektrisch integriert. Neben der Hardware lag bereits ein erstes Architekturkonzept für die Umsetzung autonomer Fahrfunktionen vor. Dieses definierte grundlegende Komponenten für Sensorik, Fahrzeugsteuerung und Kommunikation zwischen den Softwaremodulen. Die Architektur befand sich jedoch noch auf konzeptioneller Ebene und war nicht vollständig implementiert. Darüber hinaus existierte bereits eine Smartphone-Applikation auf Basis des Flutter-Frameworks. Die Anwendung ermöglichte die Verbindung zum Fahrzeug und bildete die Grundlage für dessen manuelle Steuerung. Funktionen für autonomes Fahren, Hinderniserkennung oder automatische Fahrentscheidungen waren zu diesem Zeitpunkt jedoch noch nicht vorhanden. Im Softwarebereich bestanden außerdem erste Ansätze für die Verarbeitung von \gls{lidar}-Daten. Diese Komponenten arbeiteten jedoch ausschließlich mit statischen Test- und Beispieldaten und waren nicht mit der realen Sensorhardware gekoppelt. Eine operative Nutzung des \gls{lidar}s im Fahrbetrieb war daher noch nicht möglich. Zusammenfassend stand zu Beginn der Arbeit bereits eine technische Basis aus Fahrzeug, Sensorik, Softwarearchitektur und mobiler Anwendung zur Verfügung. Die vorhandenen Komponenten waren jedoch nur teilweise funktionsfähig und boten noch keine autonome Fahrfunktion.
...
\\
Zweitens funktionierten alle Sensoren bis auf einen nicht. Nach der Aktualisierung des \gls{os} auf dem Raspberry~Pi brach die gesamte \gls{gpio}-Ansteuerung, wodurch neben Motorsteuerung und Lenkung auch die Sensorik ausfiel. Der Ultraschallsensor lieferte zusätzlich aufgrund eines Pinkonflikts mit dem \gls{spi}-Subsystem keine gültigen Messwerte. Zeitweise stand somit nur ein einziger funktionsfähiger Sensor zur Verfügung. Die Analyse und Behebung dieser Ausfälle sowie der daraus abgeleitete Wechsel auf den \gls{lidar} als primären Sensor werden in Abschnitt~\ref{sec:datenverarbeitung_sonia} beschrieben.
\\
